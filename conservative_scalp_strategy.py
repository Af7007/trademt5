#!/usr/bin/env python3
"""
ESTRATÉGIA DE SCALPING CONSERVADOR PARA BTCUSDc (24/7)
...
class ConservativeScalpStrategy:
    """Estratégia de scalping ultra-conservador para BTCUSDc (24/7)"""

    def __init__(self):
        # Configurações da estratégia
        self.symbol = "BTCUSDc"
        self.timeframe = mt5.TIMEFRAME_M1  # 5 minutos
        ...
        # Parâmetros técnicos (ajustados para BTC mercado ultra-volátil atual)
        self.sl_points = 80   # Stop Loss: 80 pontos (mercado muito volátil)
        self.tp_points = 16   # Take Profit: 16 pontos (mercado muito volátil)
        self.ema_fast = 21
        self.ema_slow = 50
        self.bb_period = 20
        self.bb_deviations = 2
        self.rsi_period = 14
        self.rsi_neutral_low = 40   # RSI expandido: 40-60 (mercado volátil)
        self.rsi_neutral_high = 60  # RSI mais permissivo no mercado alto
    ...
    def check_range_conditions(self, df):
        ...
        # Range: canal de 200-1200 pontos (BTC muito volátil atualmente - mercado atual ~781 pontos)
        bb_range = last['BB_upper'] - last['BB_lower']
        if not (200 <= bb_range <= 1200):
        ...
        # Sem gaps grandes (mais permissivo no BTC volátil)
        prev_close = df.iloc[-2]['close']
        gap = abs(last['open'] - prev_close)
        if gap > 20:  # 20 pontos max gap para BTC volátil
    ...
    def calculate_lot_size(self):
        ...
        pip_value = 0.01  # Valor por pip no BTCUSD
        ...
        logger.info("Lot calculado: {:.3f} (risco base ${:.2f})".format(final_lot, risk_per_trade))
    ...
    def analyze_current_market(self, df):
        ...
        logger.info("Range BB Target: 200-1200 pontos (BTC volátil)")
        logger.info("RSI Target: 40-60 (mercado volátil)")
        logger.info("Objetivo: ${:.2f} lucro por operacao".format(self.operation_target))
        ...
        try:
            ...
            # Range check - ajustado para volatilidade extrema do BTC atual
            range_ok = 200 <= bb_range <= 1200
            logger.info("OK Range BB (200-1200): {:.2f} -> {}".format(bb_range, "PASSOU" if range_ok else "REJEITOU"))
